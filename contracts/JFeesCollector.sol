...
contract JFeesCollector is OwnableUpgradeable, ReentrancyGuardUpgradeable, JFeesCollectorStorage, IJFeesCollector {
    ...
    */
    ...
    receive() external payable {
        emit AvaxReceived(msg.sender, msg.value, block.number);
    }

    /**
    * @dev withdraw avax amount
    * @param _amount amount of withdrawed avax
    */
    function avaxWithdraw(uint256 _amount) external onlyAdmins nonReentrant {
        require(_amount <= address(this).balance, "Not enough contract balance");
        TransferAVAXHelper.safeTransferAVAX(msg.sender, _amount);
        emit AvaxWithdrawn(_amount, block.number);
    }
