...
contract JTrancheBToken is Ownable, ERC20, AccessControl, IJTrancheTokens {
	...
	function _transfer(address from, address to, uint256 value) internal override {
        ...
        if (rewClaimCompleted && value > 0) {
			...
			uint256 stkDetNum = IJBenQi(jBQAddress).getSingleTrancheUserStakeCounterTrB(from, protTrancheNum);
			for (uint256 i = 1; i <= stkDetNum; i++){
				(tempTime, tempAmount) = IJBenQi(jBQAddress).getSingleTrancheUserSingleStakeDetailsTrB(from, protTrancheNum, i);
				if (tempAmount > 0) {
					if (tempAmount <= tempValue) {
						IJBenQi(jBQAddress).setTrBStakingDetails(protTrancheNum, from, i, 0, tempTime);
						IJBenQi(jBQAddress).setTrBStakingDetails(protTrancheNum, to, i, tempAmount, block.timestamp);
						tempValue = tempValue.sub(tempAmount);
					} else {
						uint256 remainingAmount = tempAmount.sub(tempValue);
						IJBenQi(jBQAddress).setTrBStakingDetails(protTrancheNum, from, i, remainingAmount, tempTime);
						IJBenQi(jBQAddress).setTrBStakingDetails(protTrancheNum, to, i, tempValue, block.timestamp);
						tempValue = 0;
					}
				}
				if (tempValue == 0)
